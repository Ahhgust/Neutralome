...
# Note: Uncomment these if using nonhuman great apes
chroms <- c(
'1',
#'2A',
#'2B',
...
for (i in chroms) {
    ...
    len <- length(tbl[,1])
    tbl <- tbl[ sort(sample(len, round(len/samplingFraction))),]
...
x <- 1
for (i in chroms) {
    s <- s + sum_sq_comb_gmap(posList[[x]], 
    pidList[[x]], bed1=exonBedList[[x]], 
    bed2=nonexonBedList[[x]], y, mult=T, max1=1e6,
    max2=1e6,trunc=T)
    x <- x + 1
}
