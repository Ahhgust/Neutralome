...
// TODO: check pos is in genetic map, gSTart gEnd in genetic map!
/*
  This function was written by August Woerner.
  It is the analog to do_exp_sums (above), but it takes in an additional
  set of coordinates; *gStart and *gEnd
  While *start and *end are positions in the physical map (usefuly for computing lengths)
  *gStart and *gEnd are positions in the genetic map. ie, they are the positions of 
  *start and *end in the genetic map. 
  This makes computations on linkage much more straight forward
*/
